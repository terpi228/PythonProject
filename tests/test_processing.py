from src.processing import filter_by_state, sort_by_date  # замени your_module на имя твоего файла
...
def test_filter_by_state_executed(sample_operations: List) -> None:
    ...
    # Проверяем что вернулось 3 executed операции
    assert len(result) == 3
    # Проверяем что ВСЕ операции имеют статус EXECUTED
    assert all(op["state"] == "EXECUTED" for op in result)
    # Проверяем что ID правильные
...
def test_sort_by_date_descending(sample_operations: List) -> None:
    ...
    # Проверяем порядок дат (от новых к старым)
